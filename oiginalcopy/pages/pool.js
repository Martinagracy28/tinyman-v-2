...
function Pool() {
    ...
    const mint = async (appid) => {
      ...
      let total;
      if (s1 === undefined || s2 === "") {
        total = Math.sqrt((input1 * 1000000)  * (1000000 *input2)) - 1000;
        console.log("Total,: ", total);
      } else {
        let liquidity_asset_amount = Math.min(
          ((input1 * 1000000) * ilt) / s1,
          ((input2 * 1000000) * ilt) / s2
        );
      ...
      try {
        ...
        const transaction3 =
          algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
            ...
            note: undefined,
            accounts: sender,
            amount: parseInt((input1 * 1000000)),
            suggestedParams: params,
          });
        ...
        const transaction4 =
          algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
            ...
            note: undefined,
            accounts: sender,
            amount: parseInt(input2 * 1000000),
            suggestedParams: params,
          });
        ...
        alert("Pair created",JSON.stringify(response, null, 1))
      } catch (err) {
        console.error(err);
      }
    };
    ...
    let accountasset1 = await algodClient.getAssetByID(tokenid1).do();
      ...
      let unit2 =accountasset2.params['unit-name']
        ...
        try {
          ...
          appArg.push(algosdk.encodeUint64(parseInt(tokenid1)));
          appArg.push(algosdk.encodeUint64(parseInt(tokenid2)));
          let foreignassets = [];
          foreignassets.push(parseInt(tokenid1));
          foreignassets.push(parseInt(tokenid2));
          ...
          console.log("TxID", JSON.stringify(response, null, 1));
          await waitForConfirmation(algodClient, response.txId);
          alert("Paired",JSON.stringify(response, null, 1))
          setShowOptInButton(true);
          //setShowOptInButton(true);
        } catch (err) {
          console.error(err);
  ...
  console.log("TxID", JSON.stringify(response, null, 1));
  alert("optin completed",JSON.stringify(response, null, 1))
  await waitForConfirmation(algodClient, response.txId);
